...
function Header({ onLogout, user, handleUpdateName }) {
  const handleLogout = () => {
      ...
      <div>
        ...
        <nav className="navbar navbar-expand-lg navbar-dark bg-dark">
          <div className="container px-5">
            ...
            {user && (
              <>
                <a className="navbar-brand" href="index.html">Selamat datang, {user.name}!</a>
                <a className="navbar-brand"  data-bs-toggle="modal" href="#profileModal">Profil</a>
                <UserProfile user={user} id={user.id} onUpdateName={handleUpdateName} /> {/* Meneruskan prop onUpdateName ke UserProfile */}
              </>
            )}
